...
//Phone size
canvas.height = wih * 0.9;
if (screen.width <= 500) {
  canvas.width = screen.width;
  tutorialImgEl.style.height = screen.width;
  tutorialEl.style.width = screen.width;
} else {
  canvas.width = wih * 0.5;
  tutorialImgEl.style.height = wih + "px";
  tutorialEl.style.width = wih * 0.5 + 'px';
}
tutorialEl.style.height = wih * 0.9 + "px";
...
const trext = ["хүнс", "цаас", "хуван", "шил", "мет", "элек"];
const trext2 = [" ", " ", "цар", " ", "алл", "трон"];
...
class Trash {
  ...
  draw() {
    ...
    c.font = trashWidth * 0.2 + "px Arial";
    c.textAlign = "center";
    c.fillText(
      this.name,
      this.position.x + trashWidth * 0.46,
      this.position.y + trashHeight * this.trosition
    );
    c.fillText(
      this.name2,
      this.position.x + trashWidth * 0.46,
      this.position.y + trashHeight * (this.trosition + 0.2)
    );
  }
}
